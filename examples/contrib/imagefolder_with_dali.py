...
def iter_setup(samples, batch_size):
    ...
    def read_label(label):
        # WARNING: Becareful not using [] here results in a silent quit of the script
...
def finetune_model(model, out_features, requires_grad=False):
    ...
    if name == "ResNet":
        ...
        model.classifier[1] = nn.Conv2d(
            512, out_features, kernel_size=(1, 1), stride=(1, 1)
        )
    elif name == "DenseNet":
...
def trainval_split(samples, train_ratio):
    ...
    return (
        tuple(chain.from_iterable(train_samples)),
        tuple(chain.from_iterable(val_samples)),
    )
...
def run(
    ...
    train_samples, val_samples, n_categories = make_samples(
        root, local_rank, world_size, val_ratio
    )

    model, input_size = make_model(
        model_name, n_categories, requires_grad=requires_grad, pretrained=pretrained
    )
    torch.cuda.set_device(device_id)
    model.to("cuda")
    ...
    train_loader = DALILoader(
        pipe, ["data", "label"], auto_reset=True, stop_at_epoch=True
    )
    trainer = create_supervised_dali_trainer(
        model,
        optimizer,
        loss_fn,
        device=None,
    ...
    def log_training_results(engine):
        ...
        if local_rank == 0:
            ...
            samples is not the same per process so the average loss is slightly
