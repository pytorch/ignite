...
class TensorBoardwithGAN(TensorBoardX):
    def __init__(self,
                 fixed_noise,
                 netG,
                 netD):
        super().__init__(histogram_freq=1, use_output=True, write_grads=True)
        self.fixed_noise = fixed_noise
        self.netG = netG
        self.netD = netD

    def histograms(self, engine):
        self._log_histograms(engine, self.netG, add_name=True)
        self._log_histograms(engine, self.netD, add_name=True)

    def save_fake_example(self, engine):
        fake = self.netG(self.fixed_noise)
        image = vutils.make_grid(fake, padding=1, normalize=True)
        self.writer.add_image('FakeExample', image, engine.state.epoch)

    def save_real_example(self, engine):
        img, y = engine.state.batch
        image = vutils.make_grid(img, padding=1, normalize=True)
        self.writer.add_image('RealExample', image, engine.state.epoch)
...
def main(dataset, dataroot,
    ...
    tensorboard_handler = TensorBoardwithGAN(fixed_noise=fixed_noise,
                                             netG=netG,
                                             netD=netD)
    ...
    tensorboard_handler.attach(trainer)
