...
from ignite.utils import setup_logger, one_rank_only
...
def get_data_loaders(train_batch_size, val_batch_size):
    ...
    train_loader = idist.auto_dataloader(train_dataset, batch_size=train_batch_size, shuffle=True)

    val_dataset = MNIST(download=False, root=".", transform=data_transform, train=False)

    val_loader = idist.auto_dataloader(val_dataset, batch_size=val_batch_size, shuffle=False)
        ...
        if idist.rank() == 0:
            ...
            tqdm.write(
                "Training Results - Epoch: {}  Avg accuracy: {:.2f} Avg loss: {:.2f}".format(
                    trainer.state.epoch, avg_accuracy, avg_nll
                )
            )
