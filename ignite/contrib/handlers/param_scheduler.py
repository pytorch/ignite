...
class ConcatScheduler(ParamScheduler):
    ...
    def __init__(self, schedulers: List[ParamScheduler], durations: List[int], save_history: bool = False) -> None:
        ...
        tmps_list_optimizers = [s if isinstance(s, list) else [s] for s in tmp_optimizers]
        param_optimizers = list(itertools.chain(*tmps_list_optimizers))
...
def create_lr_scheduler_with_warmup(
    ...
    for param_group_index, param_group in enumerate(lr_scheduler.optimizer.param_groups):  # type: ignore[union-attr]
    ...
    combined_scheduler = ConcatScheduler(
        schedulers,  # type: ignore[arg-type]
        durations=durations,
        save_history=save_history,
    )
    ...
    if output_simulated_values is not None:
        ...
        result = ConcatScheduler.simulate_values(
            num_events=num_events,
            schedulers=schedulers,  # type: ignore[arg-type]
            durations=durations,
        )
        for i in range(num_events):
