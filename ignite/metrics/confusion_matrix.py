...
class MultiLabelConfusionMatrix(Metric):
    ...
    def update(self, output: Sequence[torch.Tensor]) -> None:
        ...
        y_total = torch.unsqueeze(torch.count_nonzero(y, axis=0), 0)
        y_pred_total = torch.unsqueeze(torch.count_nonzero(y_pred, axis=0), 0)
        tp = torch.unsqueeze(torch.count_nonzero(torch.mul(y, y_pred), axis=0), 0)
        fp = y_pred_total - tp
        fn = y_total - tp
        tn = y.shape[0] - tp - fp - fn
    ...
    def _check_input(self, output: Sequence[torch.Tensor]) -> None:
        ...
        if y.ndimension() < 2:
            raise ValueError(
                f"y must have shape (batch_size, num_classes (currently set to {self.num_classes}), ...), "
                f"but given {y.shape}"
            )
