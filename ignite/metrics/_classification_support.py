...
class ClassificationSupport(Metric):
    def __init__(self, output_transform=lambda x: x, threshold_function=None):
        if threshold_function is not None:
            ...
            self._threshold = threshold_function
        self._updated = False
        super(ClassificationSupport, self).__init__(output_transform=output_transform)
...
class PrecisionRecallSupport(ClassificationSupport):
    ...
    def update(self, output):
        ...
        correct = y * y_pred
        positives = y_pred.sum(dim=0) if self._precision_vs_recall else y.sum(dim=0)
        ...
        if self._denominator is None:
            self._denominator = positives
            self._numerator = true_positives
        else:
            self._denominator += positives
            self._numerator += true_positives
