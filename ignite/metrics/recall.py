        ...
        if self._type == "multilabel":
            if not self._average:
                self._true_positives = torch.cat(
                    [self._true_positives, true_positives],  # type: ignore[has-type] # mypy cann't determine type
                    dim=0,
                )
                self._positives = torch.cat(
                    [self._positives, actual_positives],  # type: ignore[has-type] # mypy cann't determine type
                    dim=0,
                )
            else:
