...
class SSIM3D(Metric):
    ...
    def __init__(
        ...
        kernel_size: Union[int, Sequence[int]] = (11, 11, 11),
        sigma: Union[float, Sequence[float]] = (1.5, 1.5, 1.5),
        k1: float = 0.01,
        k2: float = 0.03,
        gaussian: bool = True,
        ...
        if isinstance(kernel_size, int):
            self.kernel_size: Sequence[int] = [kernel_size, kernel_size, kernel_size]
        ...
        if isinstance(sigma, float):
            self.sigma: Sequence[float] = [sigma, sigma, sigma]
        ...
        super(SSIM3D, self).__init__(output_transform=output_transform, device=device, skip_unrolling=skip_unrolling)
        ...
        self._kernel_3d = self._gaussian_or_uniform_kernel(kernel_size=self.kernel_size, sigma=self.sigma)
    ...
    def _gaussian_or_uniform_kernel(self, kernel_size: Sequence[int], sigma: Sequence[float]) -> torch.Tensor:
        ...
        return torch.einsum("i,j,k->ijk", kernel_x.squeeze(), kernel_y.squeeze(), kernel_z.squeeze())
    ...
    def _check_type_and_shape(self, y_pred: torch.Tensor, y: torch.Tensor) -> None:
        ...
        if len(y_pred.shape) != 5 or len(y.shape) != 5:
            raise ValueError(
                f"Expected y_pred and y to have BxCxDxHxW shape. Got y_pred: {y_pred.shape} and y: {y.shape}."
            )
    ...
    def update(self, output: Sequence[torch.Tensor]) -> None:
        ...
        if self._kernel is None or self._kernel.shape[0] != nb_channel:
            self._kernel = self._kernel_3d.expand(nb_channel, 1, -1, -1, -1)
        ...
        y_pred = F.pad(y_pred, [self.pad_d, self.pad_d, self.pad_w, self.pad_w, self.pad_h, self.pad_h], mode="reflect")
        y = F.pad(y, [self.pad_d, self.pad_d, self.pad_w, self.pad_w, self.pad_h, self.pad_h], mode="reflect")
        ...
        outputs = F.conv3d(torch.cat(input_list), self._kernel, groups=nb_channel)
        ...
        self._sum_of_ssim += torch.mean(ssim_idx, (1, 2, 3, 4), dtype=double_dtype).sum().to(device=self._device)
