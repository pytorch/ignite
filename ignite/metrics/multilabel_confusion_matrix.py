...
class MultiLabelConfusionMatrix(Metric):
    ...
    For example, y_pred[i,j] = 1 denotes that the j'th class is one of the labels of the i'th sample
    as predicted.
    - `y` should have the following shape (batch_size, num_classes, ...) with 0s and 1s.
    For example, y[i,j] = 1 denotes that the j'th class is one of the labels of the i'th sample
    according to the ground truth.
    - both 'y' and 'y_pred' must be torch Tensors having any of the following types:
    {torch.uint8, torch.int8, torch.int16, torch.int32, torch.int64}. They must have the
    same dimensions.
