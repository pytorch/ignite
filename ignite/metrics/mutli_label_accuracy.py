...
class MultiLabelAccuracy(Metric):
    ...
    - `y_pred` must be in the following shape (batch_size, num_classes,...)
    - `y` must be in the following shape (batch_size, num_classes,...)
    """
    ...
    def update(self, output):
        ...
        correct = [torch.equal(y_i, y_pred_i) for y_i, y_pred_i in zip(y, indices)]
