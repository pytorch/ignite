...
class Engine(object):
    ...
    def run(self, data, max_epochs=1):
        ...
        state
        """
        state = State(dataloader=data, epoch=0, max_epochs=max_epochs, metrics={})

        try:
            self._logger.info("Run starting with max_epochs={}".format(max_epochs))
            start_time = time.time()
            ...
            time_taken = time.time() - start_time
            hours, mins, secs = _to_hours_mins_secs(time_taken)
            self._logger.info("Run complete. Time taken %02d:%02d:%02d" % (hours, mins, secs))

        except BaseException as e:
            self._logger.error("Run is terminating due to exception: %s", str(e))
