...
def _test_gpu_info(device='cpu'):
    ...
    assert 'gpu info:0 mem' in engine.state.metrics
    assert 'gpu info:0 util' in engine.state.metrics

    assert isinstance(engine.state.metrics['gpu info:0 mem'], int)
    assert int(mem_report['used'] * 100.0 / mem_report['total']) == engine.state.metrics['gpu info:0 mem']

    assert isinstance(engine.state.metrics['gpu info:0 util'], int)
    assert int(util_report['gpu_util']) == engine.state.metrics['gpu info:0 util']
