...
def test_invalid_ssim():
    ...
    with pytest.raises(ValueError, match=r"Expected y_pred and y to have BxCxDxHxW shape."):
        y = y.squeeze(dim=0)
        ssim = SSIM3D(data_range=1.0)
        ssim.update((y, y))
        ssim.compute()
...
@pytest.mark.parametrize(
    ...
    [[(8, 3, 35, 35, 35), 7, False, True], [(12, 3, 14, 28, 28), 11, True, False]],
)
...
def compare_ssim_ignite_skiimg(
    ...
    *,
    ...
    ssim = SSIM3D(data_range=data_range, sigma=sigma, device=device)
    ssim.update((y_pred, y))
    ...
    print(f"shape={y_pred.shape}, shape={skimg_y.shape}, kernel_size={kernel_size}")
    skimg_ssim = ski_ssim(
        skimg_y_pred,
        skimg_y,
...
def test_ssim_device(available_device, available_device2):
    ...
    ssim = SSIM3D(data_range=data_range, sigma=sigma, device=metric_device)
...
def test_ssim_variable_batchsize(available_device):
    ...
    sigma = 1.5
    data_range = 1.0
    ssim = SSIM3D(data_range=data_range, sigma=sigma, device=available_device)

    y_preds = [
        torch.rand(6, 3, 14, 14, 14, device=available_device),
        torch.rand(6, 3, 14, 14, 14, device=available_device),
        torch.rand(8, 3, 14, 14, 14, device=available_device),
        torch.rand(3, 3, 14, 14, 14, device=available_device),
        torch.rand(1, 3, 14, 14, 14, device=available_device),
        torch.rand(6, 3, 14, 14, 14, device=available_device),
    ]
...
def test_ssim_variable_channel(available_device):
    y_preds = [
        torch.rand(12, 5, 14, 14, 14, device=available_device),
        torch.rand(12, 4, 14, 14, 14, device=available_device),
        torch.rand(12, 7, 14, 14, 14, device=available_device),
        torch.rand(12, 3, 14, 14, 14, device=available_device),
        torch.rand(12, 2, 14, 14, 14, device=available_device),
        torch.rand(12, 6, 14, 14, 14, device=available_device),
    ]
    y_true = [v * 0.8 for v in y_preds]
...
@pytest.mark.parametrize(
    "shape, kernel_size, gaussian, use_sample_covariance",
    [[(8, 3, 35, 35, 35), 7, False, True], [(12, 3, 14, 14, 14), 11, True, False]],
)
...
class TestDistributed:
    ...
    def test_integration(self, metric_device):
        ...
        y_pred = torch.rand(n_iters * batch_size, 3, 14, 14, 14, dtype=torch.float, device=device)
        y = y_pred * 0.65
        ...
        SSIM3D(data_range=1.0, device=metric_device).attach(engine, "ssim")
        ...
        engine = Engine(update)
        SSIM3D(data_range=1.0, gaussian=False, kernel_size=7, device=metric_device).attach(engine, "ssim")
    ...
    def test_accumulator_device(self, metric_device):
        ...
        ssim = SSIM3D(data_range=1.0, device=metric_device)

        assert ssim._kernel is None
        assert isinstance(ssim._kernel_3d, torch.Tensor)

        for dev in [ssim._device, ssim._kernel_3d.device]:
            assert dev == metric_device, f"{type(dev)}:{dev} vs {type(metric_device)}:{metric_device}"

        y_pred = torch.rand(2, 3, 14, 28, 28, dtype=torch.float, device=device)
        y = y_pred * 0.65
        ssim.update((y_pred, y))
