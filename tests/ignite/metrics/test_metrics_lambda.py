...
def _test_distrib_integration(device):
    ...
    def _test(metric_device):
        ...
        f1_true = f1_score(
            y_true.view(-1).cpu(), y_pred.reshape(-1, n_classes).cpu().argmax(dim=-1), average="macro"
        )
...
def _test_distrib_metrics_on_diff_devices(device):
    ...
    assert "f1" in state.metrics
    assert "ff1" in state.metrics
    f1_true = f1_score(y_true.view(-1).cpu(), y_preds.reshape(-1, n_classes).cpu().argmax(dim=-1), average="macro")
    assert f1_true == approx(state.metrics["f1"])
    assert 1.0 + f1_true == approx(state.metrics["ff1"])
