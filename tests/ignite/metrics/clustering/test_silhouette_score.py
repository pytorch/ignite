...
def test_zero_sample():
    with pytest.raises(
        NotComputableError, match="SilhouetteScore must have at least one example before it can be computed"
    ):
...
def test_single_cluster():
    with pytest.raises(NotComputableError, match="at least 2 clusters"):
        metric = SilhouetteScore(check_compute_fn=False)
...
def test_integration(n_times: int, test_case: tuple[Tensor, Tensor, int], available_device):
    ...
    m = SilhouetteScore(device=available_device)
    assert m._device == torch.device(available_device)

    m.attach(engine, "silhouette")

    data = list(range(np_features.shape[0] // batch_size))
    s = engine.run(data, max_epochs=1).metrics["silhouette"]

    np_ans = silhouette_score(np_features, np_labels)
...
class TestDistributed:
    def test_compute(self):
        ...
        for metric_device in metric_devices:
            m = SilhouetteScore(device=metric_device)
    ...
    def test_integration(self, n_epochs: int):
        ...
        for metric_device in metric_devices:
            ...
            silhouette = SilhouetteScore(device=metric_device)
            ...
            assert "silhouette" in engine.state.metrics

            res = engine.state.metrics["silhouette"]
