...
def test_single_cluster():
    with pytest.raises(NotComputableError, match="at least 2 clusters"):
        metric = DaviesBouldinScore(check_compute_fn=False)
...
def test_integration(n_times: int, test_case: tuple[Tensor, Tensor, int], available_device):
    ...
    s = engine.run(data, max_epochs=1).metrics["davies_bouldin_score"]

    np_ans = davies_bouldin_score(np_features, np_labels)
...
class TestDistributed:
    def test_compute(self):
        ...
        for metric_device in metric_devices:
            m = DaviesBouldinScore(device=metric_device)
    ...
    def test_integration(self, n_epochs: int):
        ...
        for metric_device in metric_devices:
            ...
            dbs = DaviesBouldinScore(device=metric_device)
            dbs.attach(engine, "davies_bouldin_score")
            ...
            assert "davies_bouldin_score" in engine.state.metrics

            res = engine.state.metrics["davies_bouldin_score"]
