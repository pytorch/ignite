...
def test_distrib_variable_accumulation(local_rank, distributed_context_single_node):
...
@pytest.mark.distributed
@pytest.mark.skipif(torch.cuda.device_count() < 2, reason="Skip if no GPU")
def test_distrib_average(local_rank, distributed_context_single_node):
...
@pytest.mark.distributed
@pytest.mark.skipif(torch.cuda.device_count() < 2, reason="Skip if no GPU")
def test_distrib_geom_average(local_rank, distributed_context_single_node):
...
@pytest.mark.distributed
@pytest.mark.skipif(torch.cuda.device_count() < 2, reason="Skip if no GPU")
def test_distrib_integration(local_rank, distributed_context_single_node):
