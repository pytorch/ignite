...
from ignite.trainer import Trainer, Events, create_supervised
...
def test_adding_handler_for_non_existent_event_throws_error():
    ...
    while event_name in Events.__members__.values():
        event_name = uuid.uuid4()
...
def test_adding_multiple_event_handlers():
    ...
    for handler in handlers:
        trainer.add_event_handler(Events.TRAINING_STARTED, handler)
...
def test_current_iteration_counter_increases_every_iteration():
    ...
    trainer.add_event_handler(Events.TRAINING_ITERATION_STARTED, IterationCounter())
...
def _inference(trainer, inference_data):
    trainer.inference(inference_data)


def test_current_inference_iteration_counter_increases_every_iteration():
    inference_batches = [1, 2, 3]
    ...
    class IterationCounter(object):
        ...
        def __call__(self, trainer):
            assert trainer.current_inference_iteration == self.current_iteration_count
            self.current_iteration_count += 1
            self.total_count += 1
    ...
    trainer.add_event_handler(Events.TRAINING_EPOCH_COMPLETED, _inference, inference_batches)
    trainer.add_event_handler(Events.INFERENCE_STARTING, clear_counter, iteration_counter)
    trainer.add_event_handler(Events.INFERENCE_ITERATION_STARTED, iteration_counter)

    trainer.run([1], max_epochs=max_epochs)
    assert iteration_counter.total_count == max_epochs * len(inference_batches)


def test_inference_is_not_called_by_default():
    trainer = Trainer(MagicMock(return_value=1), MagicMock())
    trainer.inference = MagicMock()

    max_epochs = 5
    trainer.run([1], max_epochs=max_epochs)
    assert trainer.inference.call_count == 0
...
def test_terminate_stops_training_mid_epoch():
    ...
    trainer.add_event_handler(Events.TRAINING_ITERATION_STARTED, end_of_iteration_handler)
...
def test_terminate_stops_trainer_when_called_during_inference():
    num_iterations_per_epoch = 10
    iteration_to_stop = 3  # i.e. part way through the 2nd inference run
    epoch_to_stop = 2
    trainer = Trainer(MagicMock(return_value=1), MagicMock(return_value=1))

    def end_of_iteration_handler(trainer):
        if (trainer.current_epoch == epoch_to_stop and trainer.current_inference_iteration == iteration_to_stop):

            trainer.terminate()

    trainer.add_event_handler(Events.TRAINING_EPOCH_COMPLETED, _inference, [None] * num_iterations_per_epoch)
    trainer.add_event_handler(Events.INFERENCE_ITERATION_STARTED, end_of_iteration_handler)
    ...
    assert trainer.current_inference_iteration == iteration_to_stop + 1
    assert trainer.current_iteration == (epoch_to_stop + 1) * num_iterations_per_epoch


def test_terminate_after_training_iteration_skips_inference_run():
    num_iterations_per_epoch = 10
    ...
    trainer.add_event_handler(Events.TRAINING_EPOCH_COMPLETED, _inference, MagicMock())
    trainer.add_event_handler(Events.TRAINING_ITERATION_STARTED, end_of_iteration_handler)
...
def test_training_iteration_events_are_fired():
    ...
    trainer.add_event_handler(Events.TRAINING_ITERATION_STARTED, iteration_started)

    iteration_complete = Mock()
    trainer.add_event_handler(Events.TRAINING_ITERATION_COMPLETED, iteration_complete)
...
def test_inference_iteration_events_are_fired():
    max_epochs = 5
    num_batches = 3
    inference_data = _create_mock_data_loader(max_epochs, num_batches)

    trainer = Trainer(MagicMock(return_value=1), MagicMock(return_value=1))

    mock_manager = Mock()
    iteration_started = Mock()
    trainer.add_event_handler(Events.INFERENCE_ITERATION_STARTED, iteration_started)

    iteration_complete = Mock()
    trainer.add_event_handler(Events.TRAINING_EPOCH_COMPLETED, _inference, inference_data)
    trainer.add_event_handler(Events.INFERENCE_ITERATION_COMPLETED, iteration_complete)
...
def test_inference_iteration_events_are_fired_when_inference_is_called_explicitly():
    max_epochs = 5
    num_batches = 3
    inference_data = _create_mock_data_loader(max_epochs, num_batches)
...
def test_create_supervised():
    ...
    y_pred, y = trainer.inference_history[0]
...
def test_on_decorator():
    ...
    @trainer.on(Events.TRAINING_ITERATION_STARTED, started_counter)
    def handle_training_iteration_started(trainer, started_counter):
        started_counter.count += 1

    completed_counter = Counter()

    @trainer.on(Events.TRAINING_ITERATION_COMPLETED, completed_counter)
