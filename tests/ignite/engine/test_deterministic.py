    ...
    def _test(epoch_length=None):
        ...
        for resume_iteration in range(2, min(num_iters * max_epochs, epoch_length * max_epochs), 4):
            ...
            def update_fn(_, batch):
                assert batch_checker.check(batch), f"{resume_iteration} | {batch_checker.counter}: {batch_checker.true_batch} vs {batch}"
...
def test_strict_resume_from_epoch():
    def _test(epoch_length=None):
        ...
        for resume_epoch in range(1, max_epochs):
            ...
            def update_fn(_, batch):
                assert batch_checker.check(batch), f"{resume_epoch} | {batch_checker.counter}: {batch_checker.true_batch} vs {batch}"
    ...
    def _test(epoch_length=None):
        ...
        for resume_epoch in range(1, max_epochs, 2):
            ...
            for num_workers in [0, 4]:
                ...
                def update_fn(_, batch):
                    batch_to_device = batch.to(device)
                    assert batch_checker.check(batch), f"{num_workers} {resume_epoch} | {batch_checker.counter}: {batch_checker.true_batch} vs {batch}"
    ...
    def _test(epoch_length=None):
        ...
        for resume_iteration in range(2, min(num_iters * max_epochs, epoch_length * max_epochs), 13):
            ...
            for num_workers in [0, 4]:
                ...
                def update_fn(_, batch):
                    batch_to_device = batch.to(device)
                    assert batch_checker.check(batch), f"{num_workers} {resume_iteration} | {batch_checker.counter}: {batch_checker.true_batch} vs {batch}"
                ...
                engine.run(resume_dataloader)
                assert engine.state.epoch == max_epochs
                assert engine.state.iteration == epoch_length * max_epochs, f"{num_workers}, {resume_iteration} | {engine.state.iteration} vs {epoch_length * max_epochs}"
...
def _test_resume_random_data_iterator_from_iter(device):
    def _test(epoch_length=None):
        ...
        for resume_iteration in range(1, min(num_iters * max_epochs, epoch_length * max_epochs), 7):
            ...
            engine.run(infinite_data_iterator())
            assert engine.state.epoch == max_epochs
            assert engine.state.iteration == epoch_length * max_epochs, f"{resume_iteration} | {engine.state.iteration} vs {epoch_length * max_epochs}"
