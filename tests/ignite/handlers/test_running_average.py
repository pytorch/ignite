...
from ignite.handlers import RunningAverage
...
def test_wrong_input_args():
    with pytest.raises(TypeError):
        _ = RunningAverage([12, 34])

    with pytest.raises(ValueError):
        _ = RunningAverage("output", alpha=-1.0)

    with pytest.raises(ValueError):
        _ = RunningAverage(CategoricalAccuracy(), output_transform=lambda x: x ** 2)
...
def test_integration():
    ...
    avg_output = RunningAverage('output', output_transform=lambda x: x[0], alpha=alpha)
    ...
    def manual_running_avg_acc(engine):
        ...
        batch_acc = num_correct / num_examples
