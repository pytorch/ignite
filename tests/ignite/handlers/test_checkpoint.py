...
def test_args_validation(dirname):
    ...
    with pytest.raises(ValueError):
        h = ModelCheckpoint(existing, _PREFIX, create_dir=False)
    ...
    with pytest.raises(ValueError):
        h = ModelCheckpoint(
            nonempty, _PREFIX, score_function="score", save_interval=42)

    with pytest.raises(ValueError):
        h = ModelCheckpoint(
            os.path.join(dirname, 'non_existing_dir'),
            _PREFIX,
            create_dir=False,
            save_interval=42)
...
def test_simple_recovery_from_existing_non_empty(dirname):
    previous_fname = os.path.join(dirname, '{}_{}_{}.pth'.format(
        _PREFIX, 'obj', 1))
...
def test_atomic(dirname):
    ...
    _test_existance(
        atomic=False, name='nonatomic_OK', obj=serializable, expected=True)
    _test_existance(
        atomic=False,
        name='nonatomic_FAIL',
        obj=non_serializable,
        expected=True)

    _test_existance(
        atomic=True, name='atomic_OK', obj=serializable, expected=True)
    _test_existance(
        atomic=True, name='atomic_FAIL', obj=non_serializable, expected=False)


def test_last_k(dirname):
    h = ModelCheckpoint(
        dirname, _PREFIX, create_dir=False, n_saved=2, save_interval=2)
    to_save = {'name': 42}

    for _ in range(8):
        h(None, to_save)

    expected = ['{}_{}_{}.pth'.format(_PREFIX, 'name', i) for i in [6, 8]]
...
def test_with_engine(dirname):
    ...
    handler = ModelCheckpoint(
        dirname, _PREFIX, create_dir=False, n_saved=2, save_interval=1)
