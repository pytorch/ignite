...
def _test_auto_model(model, ws, device, sync_bn=False):
    ...
    assert all([p.device.type == device for p in model.parameters()]), f"{[p.device.type for p in model.parameters()]} vs {device}"
...
def test_dist_proxy_sampler():
    ...
    for seed in range(100):
        ...
        assert set_indices_per_rank == set_true_indices, f"{set_true_indices - set_indices_per_rank} | {set_indices_per_rank - set_true_indices}"
